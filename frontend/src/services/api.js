...
export async function subirCapitulo({ audio, fecha_sermon, titulo }, onUploadProgress) {
  ...
  const { data } = await api.post('/capitulos', formData, {
    ...
    onUploadProgress: (progressEvent) => {
      ...
      const porcentaje = total ? Math.round((progressEvent.loaded * 100) / total) : 0;
      onUploadProgress(porcentaje);
    },
  });
  return data;
}
...
export async function actualizarMetadatosCapitulo(id, { titulo, fecha_sermon }) {
  const { data } = await api.patch(`/capitulos/${id}/metadatos`, { titulo, fecha_sermon });
  return data;
}

export async function eliminarCapitulo(id) {
  const { data } = await api.delete(`/capitulos/${id}`);
  return data;
}
...
export async function obtenerEstadoTrabajo(capituloId, trabajoId) {
  const { data } = await api.get(`/capitulos/${capituloId}/trabajos/${trabajoId}`);
  return data.trabajo;
}
...
export async function despromoverCapitulo(id) {
  const { data } = await api.delete(`/capitulos/${id}/promover`);
  return data;
}
