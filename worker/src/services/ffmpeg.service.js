...
function ejecutarComando(comando, args) {
  return new Promise((resolve, reject) => {
    const bin = comando === 'ffmpeg' ? FFMPEG_BIN : comando === 'ffprobe' ? FFPROBE_BIN : comando;
    const proceso = spawn(bin, args);
    let stderr = '';
...
function obtenerDuracion(filePath) {
  return new Promise((resolve, reject) => {
    const proceso = spawn(FFPROBE_BIN, [
      '-v', 'error',
 ...
 * @param {string} inputPath - ruta del audio original
